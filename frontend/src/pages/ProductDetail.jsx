    ...
    return [1,2,3,4,5].map(star => (
      <span key={star}
        ...
        style={{
          ...
          color: star <= (interactive ? rating : r) ? '#f59e0b' : '#d1d5db',
    ...
    <div style={{ background:'#f8fafc', minHeight:'100vh' }}>
      ...
      <div style={{ background:'#fff', borderBottom:'1px solid #e2e8f0', padding:'12px 0' }}>
        <div className="container" style={{ display:'flex', alignItems:'center', gap:8, fontSize:13, color:'#64748b' }}>
          ...
          <span>›</span>
          <span style={{ color:'#334155', fontWeight:600, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:200 }}>
            {product.name}
          </span>
        </div>
      </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr', gap:32 }} className="product-detail-grid">
            ...
            <div style={{
              ...
              {product.featured && (
                <div style={{ position:'absolute', top:16, left:16 }}>
                  <span style={{ background:'linear-gradient(135deg,#f59e0b,#f97316)', color:'#fff', padding:'6px 14px', borderRadius:20, fontSize:12, fontWeight:700 }}>
                    ⭐ Featured
                  </span>
                </div>
              )}
              {!inStock && (
                <div style={{ position:'absolute', inset:0, background:'rgba(255,255,255,.8)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <span style={{ background:'#ef4444', color:'#fff', padding:'10px 24px', borderRadius:12, fontWeight:800, fontSize:18 }}>
                    Out of Stock
                  </span>
                </div>
              )}
                ...
                {images.map((img, i) => (
                  <div key={i} onClick={() => setMainImg(img)}
                    style={{
                      ...
                      border:`2px solid ${mainImg===img?'#2563eb':'#e2e8f0'}`,
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:8, marginBottom:12, flexWrap:'wrap' }}>
              {product.category && (
                <span style={{ background:'#eff6ff', color:'#2563eb', padding:'4px 12px', borderRadius:20, fontSize:12, fontWeight:700 }}>
                  {product.category}
                </span>
              )}
              {product.brand && (
                <span style={{ background:'#f0fdf4', color:'#166534', padding:'4px 12px', borderRadius:20, fontSize:12, fontWeight:700 }}>
                  {product.brand}
                </span>
              )}
            </div>

            <h1 style={{ fontSize:'clamp(22px,3vw,32px)', fontWeight:900, color:'#0f172a', marginBottom:12, lineHeight:1.3 }}>
              {product.name}
            </h1>
            ...
            <div style={{ marginBottom:20 }}>
              <p style={{ fontSize:36, fontWeight:900, color:'#2563eb', lineHeight:1 }}>
              ...
              <div style={{ marginBottom:24 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:0, width:'fit-content', border:'1.5px solid #e2e8f0', borderRadius:12, overflow:'hidden' }}>
                  <button onClick={() => setQty(q => Math.max(1,q-1))}
                    style={{ width:44, height:44, border:'none', background:'#f8fafc', fontSize:20, cursor:'pointer', fontWeight:700, color:'#334155' }}>
                    −
                  </button>
                  <span style={{ width:44, height:44, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:800, fontSize:16, borderLeft:'1px solid #e2e8f0', borderRight:'1px solid #e2e8f0' }}>
                    {qty}
                  </span>
            ...
            <div style={{ display:'flex', gap:12, flexWrap:'wrap', marginBottom:24 }}>
              <button onClick={handleAddToCart} disabled={!inStock}
                style={{
                  flex:1, minWidth:140, padding:'14px 24px', borderRadius:12, border:'none',
                  fontWeight:800, fontSize:15, cursor:inStock?'pointer':'not-allowed',
                  background: added ? 'linear-gradient(135deg,#10b981,#059669)' : inStock ? 'linear-gradient(135deg,#2563eb,#7c3aed)' : '#e2e8f0',
                  color: inStock ? '#fff' : '#94a3b8',
                  boxShadow: inStock ? '0 4px 14px rgba(37,99,235,.3)' : 'none',
                  transition: 'all .2s',
                }}>
              ...
              <button onClick={handleBuyNow} disabled={!inStock}
                style={{
                  flex:1, minWidth:140, padding:'14px 24px', borderRadius:12,
                  border:'2px solid #2563eb', fontWeight:800, fontSize:15,
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
              {[
                ...
                ['💯','Authentic','100% original products'],
              ].map(([icon, title, sub]) => (
          ...
          <div style={{ display:'flex', borderBottom:'1px solid #e2e8f0', overflowX:'auto' }}>
            {[
              ...
              <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                style={{
                  padding:'14px 24px', border:'none', fontWeight:700, fontSize:14, cursor:'pointer',
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr', gap:2 }}>
                {[
                  ...
                  <div key={k} style={{
                    ...
                    background: i%2===0?'#f8fafc':'#fff', borderRadius:8,
                  }}>
                ...
                <div style={{ background:'#f8fafc', borderRadius:16, padding:20, marginBottom:24, border:'1px solid #e2e8f0' }}>
                  ...
                  <textarea value={comment} onChange={e => setComment(e.target.value)}
                    ...
                    rows={3} style={{
                      ...
                      marginBottom:12, boxSizing:'border-box', lineHeight:1.6,
                    }} />
                  <button onClick={handleReview} disabled={reviewing || !comment.trim()}
                    style={{
                      padding:'10px 24px', borderRadius:10, border:'none',
                      background: reviewing ? '#e2e8f0' : 'linear-gradient(135deg,#2563eb,#7c3aed)',
                      color: reviewing ? '#94a3b8' : '#fff', fontWeight:700, fontSize:14, cursor:'pointer',
                    }}>
