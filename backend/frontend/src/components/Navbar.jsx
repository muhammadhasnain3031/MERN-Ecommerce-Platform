...
export default function Navbar() {
  ...
  const handleLogout = () => {
    dispatch(logout()); navigate('/login'); setMob(false);
  };
        ...
        <div className="container" style={{ display:'flex', alignItems:'center', height:64, gap:16 }}>
            ...
            <div style={{ position:'relative' }}>
              <input value={search} onChange={e => setSearch(e.target.value)}
                ...
                style={{
                  ...
                  borderRadius:24, border:'1.5px solid rgba(255,255,255,.2)',
          ...
          <div style={{ display:'none', alignItems:'center', gap:4 }} className="nav-desk">
            {NAV.map(n => (
              <Link key={n.to} to={n.to} style={{
                color:      isActive(n.to) ? '#fbbf24' : 'rgba(255,255,255,.8)',
                fontWeight: isActive(n.to) ? 700 : 500,
                fontSize:   14, padding:'6px 12px', borderRadius:8,
                background: isActive(n.to) ? 'rgba(255,255,255,.1)' : 'transparent',
                transition: 'all .2s',
              }}>{n.label}</Link>
            ))}
            {user?.isAdmin && (
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, marginLeft:'auto' }} className="nav-mob">
            ...
            <button onClick={() => setMob(!mob)} style={{
              background:'none', color:'#fff', fontSize:26, padding:4,
            }}>
              {mob ? '✕' : '☰'}
            </button>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
              {[
                ...
                <Link key={l.to} to={l.to} onClick={() => setMob(false)} style={{
                  display:'flex', alignItems:'center', gap:14,
                  color: isActive(l.to) ? '#fbbf24' : 'rgba(255,255,255,.85)',
                  padding:'13px 12px', borderRadius:10, fontSize:15, fontWeight:500,
                  background: isActive(l.to) ? 'rgba(255,255,255,.08)' : 'transparent',
                  transition:'all .15s',
                }}>
              ...
              {user?.isAdmin && (
                <Link to="/admin" onClick={() => setMob(false)} style={{
                  display:'flex', alignItems:'center', gap:14,
                  color:'#fbbf24', padding:'13px 12px', borderRadius:10, fontSize:15, fontWeight:700,
                  background:'rgba(245,158,11,.1)', border:'1px solid rgba(245,158,11,.2)', marginTop:4,
                }}>
            ...
            <div style={{ marginTop:24 }}>
              {user ? (
                <button onClick={handleLogout} style={{
                  ...
                  border:'1px solid rgba(239,68,68,.3)', fontWeight:700, fontSize:14,
                }}>🚪 Logout</button>
              ) : (
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  <Link to="/login" onClick={() => setMob(false)} style={{
                    ...
                    border:'1px solid rgba(255,255,255,.2)', borderRadius:12, fontWeight:600,
                  }}>Login</Link>
                  <Link to="/register" onClick={() => setMob(false)} style={{
                    display:'block', textAlign:'center', padding:12,
                    background:'linear-gradient(135deg,#f59e0b,#f97316)',
